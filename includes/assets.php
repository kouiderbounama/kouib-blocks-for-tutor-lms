...
/**
 * Returns a cache-busting version for a registered asset.
 *
 * Uses the file's modification time (filemtime) so that whenever an asset file
 * is edited on disk the enqueued URL changes (?ver=...) and browsers reload it
 * — without needing a manual plugin version bump. Falls back to KOUIB_VERSION
 * when the file cannot be stat'd.
 */
function kouib_asset_version( $relative_path ) {
	$file = trailingslashit( dirname( __DIR__ ) ) . ltrim( $relative_path, '/' );
	$mtime = @filemtime( $file );
	return ( false !== $mtime ) ? (string) $mtime : KOUIB_VERSION;
}

function kouib_register_assets() {
	if ( ! wp_style_is( 'kouib-style', 'registered' ) ) {
		wp_register_style( 'kouib-style', KOUIB_URL . 'assets/css/kouib.css', array(), kouib_asset_version( 'assets/css/kouib.css' ) );
	}

	if ( ! wp_script_is( 'kouib-view', 'registered' ) ) {
		wp_register_script( 'kouib-view', KOUIB_URL . 'assets/js/view.js', array(), kouib_asset_version( 'assets/js/view.js' ), true );
	}

	// Carousel block assets
	if ( ! wp_style_is( 'kouib-carousel-style', 'registered' ) ) {
		wp_register_style( 'kouib-carousel-style', KOUIB_URL . 'assets/css/kouib-carousel.css', array(), kouib_asset_version( 'assets/css/kouib-carousel.css' ) );
	}

	if ( ! wp_script_is( 'kouib-carousel-view', 'registered' ) ) {
		wp_register_script( 'kouib-carousel-view', KOUIB_URL . 'assets/js/carousel.js', array(), kouib_asset_version( 'assets/js/carousel.js' ), true );
	}

	// Categories grid block assets
	if ( ! wp_style_is( 'kouib-categories-style', 'registered' ) ) {
		wp_register_style( 'kouib-categories-style', KOUIB_URL . 'assets/css/kouib-categories.css', array(), kouib_asset_version( 'assets/css/kouib-categories.css' ) );
	}

	// Quick search block assets
	if ( ! wp_style_is( 'kouib-search-style', 'registered' ) ) {
		wp_register_style( 'kouib-search-style', KOUIB_URL . 'assets/css/kouib-search.css', array(), kouib_asset_version( 'assets/css/kouib-search.css' ) );
	}

	if ( ! wp_script_is( 'kouib-search-view', 'registered' ) ) {
		wp_register_script( 'kouib-search-view', KOUIB_URL . 'assets/js/search.js', array(), kouib_asset_version( 'assets/js/search.js' ), true );
	}

	// Platform stats block assets (CSS only — no JavaScript in the first release)
	if ( ! wp_style_is( 'kouib-stats-style', 'registered' ) ) {
		wp_register_style( 'kouib-stats-style', KOUIB_URL . 'assets/css/kouib-stats.css', array(), kouib_asset_version( 'assets/css/kouib-stats.css' ) );
	}
}
