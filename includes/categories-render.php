 ...
 * archive. It borrows the app helpers kouib_clamp() and kouib_sanitize_primary_color()
 * defined in carousel-render.php. The cache shares the common kouib_ prefix
 * (kouib_categories_ key) and is registered as an active key so it is
 * invalidated automatically along with the shared invalidation triggers.
 */
...
function kouib_render_courses_categories( $attributes ) {
	...
	if ( ! $is_editor_request ) {
		...
		$cache_key = 'kouib_categories_' . KOUIB_VERSION . '_' . md5( wp_json_encode( $cache_attributes ) );
	...
	$columns        = isset( $attributes['columns'] ) ? kouib_clamp( $attributes['columns'], 1, 6 ) : 3;
	$columns_tablet = isset( $attributes['columnsTablet'] ) ? kouib_clamp( $attributes['columnsTablet'], 1, 6 ) : 2;
	$columns_mobile = isset( $attributes['columnsMobile'] ) ? kouib_clamp( $attributes['columnsMobile'], 1, 6 ) : 1;
