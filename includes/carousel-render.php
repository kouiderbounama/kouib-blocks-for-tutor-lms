 ...
 * rendering kouib_render_course_card(). The cache uses its own kouibc_html_ prefix
 * and is registered as an active key, so it is automatically flushed with the
 * rest of the plugin's cache.
 */
 ...
 */
function kouibc_clamp( $value, $min, $max ) {
 ...
 */
function kouibc_sanitize_primary_color( $color ) {
...
function kouib_render_courses_carousel( $attributes ) {
	...
	if ( ! $is_editor_request && 'rand' !== $orderby_attr ) {
		...
		$cache_key = 'kouibc_html_' . KOUIB_VERSION . '_' . md5( wp_json_encode( $cache_attributes ) );
	...
	$courses_to_show  = isset( $attributes['coursesToShow'] ) ? kouibc_clamp( $attributes['coursesToShow'], 1, 24 ) : 3;
	$columns          = isset( $attributes['columns'] ) ? kouibc_clamp( $attributes['columns'], 1, 4 ) : 3;
	$columns_tablet   = isset( $attributes['columnsTablet'] ) ? kouibc_clamp( $attributes['columnsTablet'], 1, 4 ) : 2;
	$columns_mobile   = isset( $attributes['columnsMobile'] ) ? kouibc_clamp( $attributes['columnsMobile'], 1, 4 ) : 1;
	$autoplay         = ! empty( $attributes['autoplay'] );
	$autoplay_speed   = isset( $attributes['autoplaySpeed'] ) ? kouibc_clamp( $attributes['autoplaySpeed'], 1000, 10000 ) : 3000;
	$speed            = isset( $attributes['speed'] ) ? kouibc_clamp( $attributes['speed'], 100, 2000 ) : 500;
